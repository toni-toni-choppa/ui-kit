...
function Sidebar({ children, isOpen }: { children: React.ReactNode, isOpen: boolean }) {
    return (
        <aside className={`${layout.sidebar} ${isOpen ? layout['sidebar-shown'] : ''}`}>
            {children}
        </aside>
    )
}
...
function SidebarListItem({ children }: { children: React.ReactNode }) {
    return (
        <li className={layout.sidebarListItem}>
            {children}
        </li>
    )
}
